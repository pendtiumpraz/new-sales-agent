...
- ⛔ **belum (keblok creds):** deliverability lanjut (warmup/DMARC report); pengiriman live channel non-email (WA/LinkedIn); (opsional) MS Graph `Mail.Send` kalau SMTP AUTH dimatikan; simpan `send_job.email_id` buat mapping bounce yang lebih kuat
