...
- **Tick 1 (setup run)** — Batch C #1 ErrorState STARTED. Discovery: `components/shared/error-state.tsx` ALREADY EXISTS (icon/title/desc/onRetry) — the real gap is pages not USING it on `isError`. Wired it to **marketplace** (queryFn now throws on HTTP error instead of masquerading as "Marketplace nonaktif", + `isError` → `<ErrorState onRetry={refetch}/>`).
- **Tick 2** — ErrorState rollout cont'd: **admin** (tenants table) + **settings/billing**. Both already `throw` in their queryFn (isError fires) — just added the `isError` branch + `refetch`. **Next tick:** settings/{ai,mailboxes,team}, contacts/profiles, ecommerce, contacts/map, cadences, dashboard. Note: mock-hook pages (useOrders/useDeals/useContacts) need the hook to throw on `!r.ok` first — check `lib/api-mock/hooks.ts`.
