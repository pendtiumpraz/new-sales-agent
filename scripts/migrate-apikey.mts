// Additive migration runner for 0048_api_key.sql — creates the `api_key` table
// (+ indexes + RLS + tenant_isolation policy) and GRANTs it to the NOBYPASSRLS
// app role (new tables are owner-only until granted).
//   run: npx tsx scripts/migrate-apikey.mts
...
const sqlPath = resolve(process.cwd(), "drizzle/migrations/0048_api_key.sql");
...
try {
  ...
  console.log(`OK: applied ${statements.length} statement(s) from 0048_api_key.sql`);

  if (appRole) {
    await client.query(`grant select, insert, update, delete on api_key to ${appRole}`);
    console.log(`GRANTED api_key to role "${appRole}"`);
  } else {
  ...
  const { rows } = await client.query(
    `select column_name from information_schema.columns where table_name = 'api_key' order by ordinal_position`,
  );
  console.log("VERIFY api_key columns:", rows.map((r: { column_name: string }) => r.column_name).join(", "));
} finally {
  await client.end();
}
