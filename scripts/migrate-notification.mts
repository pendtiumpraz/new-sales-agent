// Additive migration runner for 0046_notification.sql — creates the `notification`
...
//   run: npx tsx scripts/migrate-notification.mts
...
const sqlPath = resolve(process.cwd(), "drizzle/migrations/0046_notification.sql");
...
try {
  ...
  console.log(`OK: applied ${statements.length} statement(s) from 0046_notification.sql`);

  if (appRole) {
    await client.query(`grant select, insert, update, delete on notification to ${appRole}`);
    console.log(`GRANTED notification to role "${appRole}"`);
  } else {
  ...
  console.log("VERIFY notification columns:", rows.map((r: { column_name: string }) => r.column_name).join(", "));
} finally {
  await client.end();
}
