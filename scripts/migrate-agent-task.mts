// Additive migration runner for 0049_agent_task.sql — creates the `agent_task`
// table (+ indexes + RLS + tenant_isolation policy) and GRANTs it to the
// NOBYPASSRLS app role (new tables are owner-only until granted).
//   run: npx tsx scripts/migrate-agent-task.mts
...
const sqlPath = resolve(process.cwd(), "drizzle/migrations/0049_agent_task.sql");
...
try {
  ...
  console.log(`OK: applied ${statements.length} statement(s) from 0049_agent_task.sql`);

  if (appRole) {
    await client.query(`grant select, insert, update, delete on agent_task to ${appRole}`);
    console.log(`GRANTED agent_task to role "${appRole}"`);
  } else {
  ...
  const { rows } = await client.query(
    `select column_name from information_schema.columns where table_name = 'agent_task' order by ordinal_position`,
  );
  console.log("VERIFY agent_task columns:", rows.map((r: { column_name: string }) => r.column_name).join(", "));
} finally {
  await client.end();
}
