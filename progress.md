...
- [x] **WAHA hosted + Vercel (per-account, 1 QR/akun)**: `lib/wa/waha.ts` di-extend (per-account session: `wahaSessionName`, `getSessionInfo`, `upsertSession`+webhook, `getQr` raw/png, `logoutSession`, `sendTextSession`, typing; env terima `WAHA_BASE_URL`/`WAHA_URL`). `/api/wa/session` GET/POST/DELETE drive WAHA langsung kalau dikonfigurasi → tiap akun bikin session sendiri + webhook per-session → scan QR sendiri. Outbound: `/api/wa/waha/inbound` kirim bubble **inline via WAHA** (typing+delay capped, `maxDuration=30`) → **gak butuh bridge**. `WaConnectCard` render QR raw/image. Secret cuma di `.env.local`+Vercel.
