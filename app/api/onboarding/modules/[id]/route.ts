...
// DELETE /api/onboarding/modules/[id]         → SOFT delete (sets deleted_at).
// DELETE /api/onboarding/modules/[id]?purge=1 → HARD delete (permanent). platform.manage.
export async function DELETE(req: Request, { params }: Ctx) {
  ...
  return handle(async () => {
    if (hard) {
      await onboardingService.hardDeleteModule(params.id, g.ctx.userId);
