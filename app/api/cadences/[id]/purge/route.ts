import { hasDb } from "@/lib/db/client";
import { requirePermission } from "@/lib/rbac/guard";

import { ok, fail, handle } from "@/modules/_shared/api";
import { outreachService } from "@/modules/outreach/service";

export const runtime = "nodejs";

interface Ctx {
  params: { id: string };
}

// DELETE|POST /api/cadences/[id]/purge → HARD delete (permanent, cascades to steps
// + enrollments). Irreversible. data.write. Alias for `DELETE /api/cadences/[id]?purge=1`.
async function purge(_req: Request, { params }: Ctx) {
  const g = await requirePermission("data.write");
  if ("error" in g) return fail("Forbidden", 403, "forbidden");
  if (!hasDb()) return fail("Database tidak tersedia", 503, "no_db");
  return handle(async () => {
    await outreachService.hardDeleteCadence(g.ctx, params.id);
    return ok({ id: params.id, purged: true });
  }, "api/cadences/[id]/purge");
}

export const DELETE = purge;
export const POST = purge;
